...
# Lokal wird neben bot.py gespeichert.
# Auf Railway wird automatisch das verbundene Volume verwendet.
...
class MarketView(discord.ui.View):
    def __init__(self, teams: list[tuple[str, list[dict]]], author_id: int):
    ...
    def make_embed(self) -> discord.Embed:
        ...
        description = f"Team {self.index + 1} von {len(self.teams)}"
        if team_role:
            description += f"\n**Team:** {team_role}"
...
class AllDriversView(discord.ui.View):
    def __init__(self, drivers: list[dict], author_id: int, per_page: int = 15):
        super().__init__(timeout=300)
...
async def drivermarket(interaction: discord.Interaction) -> None:
    ...
    view = MarketView(teams, interaction.user.id)
...
async def alldrivers(interaction: discord.Interaction) -> None:
    ...
    view = AllDriversView(sorted_drivers, interaction.user.id)
    await interaction.response.send_message(embed=view.make_embed(), view=view)
    view.message = await interaction.original_response()
...
async def on_app_command_error(
    ...
    if isinstance(error, app_commands.MissingPermissions):
        message = "❌ Dafür brauchst du die Berechtigung **Server verwalten**."
    else:
